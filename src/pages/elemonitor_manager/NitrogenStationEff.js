...
function NitrogenStationEff({ dispatch, user, monitorIndex }){
    const { authorized, theme } = user;
    const { machList, currentMach, chartInfo, isLoading } = monitorIndex;
    useEffect(()=>{
        if ( authorized ) {
            dispatch({ type:'monitorIndex/initNitrogenStation'})
        }
    },[authorized])
    const sidebar = (
                    ...
                    <div className={user.theme === 'dark' ? style['list-container'] + ' ' + style['dark'] : style['list-container']}>
                        {
                            ...
                            machList.map((item,index)=>(
                                <div key={index} className={ currentMach.mach_id === item.mach_id ? style['list-item'] + ' ' + style['selected']: style['list-item']} onClick={()=>{
                                    ...
                                    dispatch({ type:'monitorIndex/fetchFrozenChartInfo', payload:{ type:'nitrogen' }});
                                }}>
                                    { item.meter_name }
                                </div>
                            ))
    ...
    const content = (
        <div style={{ position:'relative' }}>
            ...
            <div style={{ height:'40px' }}>
                <CustomDatePicker onDispatch={()=>dispatch({ type:'monitorIndex/fetchFrozenChartInfo', payload:{ type:'nitrogen' }})} />
            </div>
            <div style={{ height:'calc( 100% - 40px)' }}>
                <div className={IndexStyle['card-container']}>
                    <ChartContainer data={chartInfo} theme={theme} type='nitrogen' />
                </div>
            </div>
        </div>
    );
...
export default connect(({ user, monitorIndex })=>({ user, monitorIndex }))(NitrogenStationEff);