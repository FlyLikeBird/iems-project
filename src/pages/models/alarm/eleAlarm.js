...
export default {
    ...
    effects:{
        ...
        *fetchRealTimeAlarm(action, { select, call, put}){
            yield put.resolve({ type:'cancelRealTimeAlarm'});
            yield put.resolve({ type:'cancelable', task:fetchRealTimeAlarmCancelable, action:'cancelRealTimeAlarm'});
            function* fetchRealTimeAlarmCancelable(params){
                try {
                    let { user:{ company_id }, eleAlarm:{ typeCode, dayTimeType }, fields:{ currentAttr }} = yield select();
                    let { nofresh } = action.payload || {};
                    if ( !nofresh ) {
                        yield put({ type:'toggleRealLoading'});
                    }
