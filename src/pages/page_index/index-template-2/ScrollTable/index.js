            ...
            <div className={style['content-container']}>
                {
                    ...
                    scroll.list.map((item,index)=>(
                        <div className={style['item']} key={index}>
                            <div><span className={style['num']}>{ scroll.current * scrollNum + ( index + 1) }</span></div>
                            <div>{ item.mach_name || '-- --'}</div>
                            <div>{ item.region_name || '-- --' }</div>
                            <div><span className={style['tag']}>{ item.type_name}</span></div>
                            <div>
                                <div>{ item.warning_date.split(' ')[1]}</div>
                                <div>{ item.warning_date.split(' ')[0]}</div>
                            </div>
                        </div>
                    ))
                    :
