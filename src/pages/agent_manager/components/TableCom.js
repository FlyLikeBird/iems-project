...
function TableCom({ dispatch, data, forWarning }){
    ...
    function handleToggleRank(item){
        new Promise((resolve, reject)=>{
            dispatch({ type: item.key === '1' ? 'agentMonitor/fetchEnergyRank' : 'agentMonitor/fetchOutputRank', payload:{ resolve, reject }})
        })
        .then(()=>{
            setCurrent(item.key);
        })
    ...
    return (
        <div style={{ height:'100%', paddingBottom:'20px'}}>
            ...
            <div style={{ height: forWarning ? '94%' : '100%', fontSize:'0.8rem' }} ref={containerRef} onMouseOver={handleMouseOver} onMouseOut={handleMouseOut}> 
                <div style={{ display:'flex', alignItems:'center', height:'9%', padding:'4px 6px' }}>
                    <div style={{ flex:'1' }}>排名</div>
                    <div style={{ flex:'2' }}>{ forWarning ? '告警种类' : '公司' }</div>
                    <div style={{ flex:'2' }}>{ forWarning ? '告警次数' : current === '1' ? '标煤排放(tce)' : current === '2' ? '万元产值比' : ''}</div>
                ...
                </div>
                {
                    ...
                    list.map((item,index)=>(
                        <div key={index} style={{ display:'flex', alignItems:'center', height:'9%', padding:'4px 6px', backgroundColor: index % 2 === 0 ? 'rgba(0,0,0,0.1)' : 'transparent' }}>
                            ...
                            <div style={{ flex:'2', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{ forWarning ? item.type_name : item.company_name }</div>
                            <div style={{ flex:'2', color:'#17e6ff' }}>{  forWarning ? item.count : Math.floor( current === '1' ? item.totalEnergy : item.ratio  ) }</div>
                            <div style={{ flex:'1', color:'#ffa633'}}>{ forWarning ? item.ratio + '%' : (+item.same).toFixed(1) + '%' }</div>
                        </div>
                    ))
                    :
                    null
