...
function EffRank({ dispatch, user, fields, carbon }){
    ...
    const sidebar = (
        <div className={style['card-container']}>
                <Tabs className={style['custom-tabs']} activeKey={energyInfo.type_id} onChange={activeKey=>{
                    ...
                    .then((node)=>{
                        dispatch({type:'carbon/fetchCarbonEff', payload:{ type:'4' }});
                        dispatch({ type:'carbon/fetchTypeRule', payload:{ warning_type:'area_effect'}})
                    })
                }}>
                    {
        ...
        <div style={{ position:'relative' }}>
            ...
            <div style={{ height:'calc( 100% - 40px)'}}>
                <div className={style['card-container']}>
                {
                    ...
                    ?                
                    <EffLineChart data={carbonEff} theme={user.theme} currentAttr={currentAttr} typeRule={typeRule} dispatch={dispatch} info={{ tab:'面积能效', key:'4', unit:'元/㎡', warning_type:'area_effect' }} startDate={user.startDate} timeType={user.timeType} />                                  
                    :
    ...
    useEffect(()=>{
        ...
        dispatch({ type:'carbon/initCarbonEff', payload:{ type:'4', warning_type:'area_effect' }});                                                     
    },[])
    return (
