...
function EffRank({ dispatch, user, fields, carbon }){
    ...
    const sidebar = (
        <div className={style['card-container']}>
                <Tabs className={style['custom-tabs']} activeKey={energyInfo.type_id} onChange={activeKey=>{
                    ...
                    .then((node)=>{
                        dispatch({type:'carbon/fetchCarbonEff', payload:{ type:'2' }});
                        dispatch({ type:'carbon/fetchTypeRule', payload:{ warning_type:'output_value_effect'}})
                    })
                }}>
                    {
        ...
        <div style={{ position:'relative' }}>
            ...
            <div style={{ height:'calc( 100% - 40px)'}}>
                <div className={style['card-container']}>
                {
                    ...
                    ?                
                    <EffLineChart data={carbonEff} theme={user.theme} typeRule={typeRule} currentAttr={currentAttr} info={{ tab:'万元产值比', key:'2', unit:'元/万元', warning_type:'output_value_effect' }} startDate={user.startDate} timeType={user.timeType} dispatch={dispatch} />                                  
                    :
    ...
    useEffect(()=>{
        ...
        dispatch({ type:'carbon/initCarbonEff', payload:{ type:'2', warning_type:'output_value_effect' }});                                                     
    },[])
    return (
