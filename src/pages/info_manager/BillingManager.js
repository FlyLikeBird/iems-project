...
function BillingManager({ dispatch, user, billing }){
        ...
        },
        {
            ...
            render: (value, row)=>{
                const renderNode = (
                    <div>
                        {
                            row.timeList.map(time=>(
                                 <div className={style['item']}><span style={{color:'#1890ff'}}>{`${time.fee_rate}元`}</span></div>
                            ))
                        }
                    </div>
                )
...
export default connect( ({ user, billing }) => ({ user, billing }))(BillingManager);